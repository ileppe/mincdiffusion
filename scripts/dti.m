...
function hh = dti(base_images, sigma)
...
% like Rohde, 2004, for robust eddy current correction and motion correction
...
THRESH=0;                            %recommend no threshold
...
[b,number_images,zeroimage]=bmatrixfromheader(base_images); 
...
if miinquire(base_images,'dimlength','yspace')==height & miinquire(base_images,'dimlength','xspace')==width 

  orientation = 't';
  slicedir = 'zspace';
 
elseif miinquire(base_images,'dimlength','zspace')==height & miinquire(base_images,'dimlength','xspace')==width 
  
  orientation = 'c';
  slicedir = 'yspace';
     
elseif miinquire(base_images,'dimlength','zspace')==height & miinquire(base_images,'dimlength','yspace')==width
...
for k=1:size(outputs,1)
  name2=strcat(name,'_',outputs(k,:),'.mnc');
...
%unix(['rgbvector ',name,'_e1x.mnc ',name,'_e1y.mnc ',name,'_e1z.mnc ',name,'_FA.mnc ',name,'_rgb.mnc']);
...
function [b,number_images,zeroimage]=bmatrixfromheader(base_images)
...
tmpfilename=['tmp' num2str(now)];

system(['mincinfo -attvalue acquisition:bvalues ' base_images ' > /tmp/' tmpfilename]);

fid=fopen(['/tmp/' tmpfilename],'r');

bvalues=fscanf(fid,'%f');

system(['rm -f /tmp/' tmpfilename]);

%these are minc x,y,z

tmpfilename=['tmp' num2str(now)];

system(['mincinfo -attvalue acquisition:direction_x ' base_images ' > /tmp/' tmpfilename]);

fid=fopen(['/tmp/' tmpfilename],'r');

directionx=fscanf(fid,'%f');

system(['rm -f /tmp/' tmpfilename]);

tmpfilename=['tmp' num2str(now)];

system(['mincinfo -attvalue acquisition:direction_y ' base_images ' > /tmp/' tmpfilename]);

fid=fopen(['/tmp/' tmpfilename],'r');

directiony=fscanf(fid,'%f');
...
system(['mincinfo -attvalue acquisition:direction_z ' base_images ' > /tmp/' tmpfilename]);

fid=fopen(['/tmp/' tmpfilename],'r');

directionz=fscanf(fid,'%f');
